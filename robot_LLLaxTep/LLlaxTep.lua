...
report = function(message) -- рапорт о состоянии
  --message = '|'..X..' '..Y..' '..Z..'|\n'..message..'\nenergy level: '..math.floor(energy_level()*100)..'%' -- добавить к сообщению координаты и уровень энергии
  if modem then -- если есть модем
	component.modem.broadcast(nopTbl.coo6LLleHu9l_oT_LLlaxTepa, serialization.serialize(message))
...
check = function(forcibly) -- проверка инструмента, батареи, удаление меток
  if not ignore_check and (steps%32 == 0 or forcibly) then -- если пройдено 32 шага или включен принудительный режим
    ...
    if robot.durability()/W_R < delta then -- если инструмент изношен
      report{"coo6LLleHue_Ha_chatBox", "инструмент изношен"}
      ignore_check = true
      home(true) -- отправиться домой
    end
    if delta*E_C > computer.energy() then -- проверка уровня энергии
      report{"coo6LLleHue_Ha_chatBox", "энергия на исходе"}
      ignore_check = true
      home(true) -- отправиться домой
    end
    if energy_level() < 0.3 then -- если энергии меньше 30%
      ...
      if generator and generator.count() == 0 and not forcibly then -- если есть генератор
		report{"coo6LLleHue_Ha_chatBox", "перезарядка генератора"}
      ...
      elseif solar and geolyzer.isSunVisible() and -- проверить видимость солнца
		...
		report{"coo6LLleHue_Ha_chatBox", "зарядка робота на солнце"}
        sorter(true)
		...
		report{"coo6LLleHue_Ha_chatBox", "возвращение к работе"}
      end
    end
  end
...
step = function(side, ignore) -- функция движения на 1 блок
  ...
  if not result and obstacle ~= 'air' and robot.detect(side) then -- если блок нельзя разрушить
	...
	report{"coo6LLleHue_Ha_chatBox", "непреодалимое припятствие"}
	stop_work()
  else
...
calibration = function() -- калибровка при запуске
  if not controller then -- проверить наличие контроллера инвентаря
	report{"coo6LLleHue_Ha_chatBox", "для работы требуется контроллер инвентаря"}
	stop_work()
  elseif not geolyzer then -- проверить наличие геосканера
	report{"coo6LLleHue_Ha_chatBox", "для работы требуется геоанализатор"}
	stop_work()
  elseif not robot.detect(0) then
	report{"coo6LLleHue_Ha_chatBox", "нижний твёрдный блок не обнаружен"}
	stop_work()
  elseif not robot.durability() then
	report{"coo6LLleHue_Ha_chatBox", "подходящий инуструмент не обнаружен"}
	stop_work()
  end
  ...
  if not D then
	report{"coo6LLleHue_Ha_chatBox", "ошибка калибровки"}
	stop_work()
  end
end
...
home = function(forcibly, interrupt) -- переход к начальной точке и сброс лута
  local x, y, z, d
  report{"coo6LLleHue_Ha_chatBox", "разгрузка руды"}
  ignore_check = true
  ...
  while true do -- войти в бесконечный цикл
    ...
    if not size or size<26 then -- если контейнер не найден
  	  report{"coo6LLleHue_Ha_chatBox", "инвентарь не найден"}
      sleep(30)
    else
  ...
  for slot = 1, inventory do -- обойти весь инвентарь
    ...
    if item then -- если слот не пуст
      if not wlist[item.name] then -- если предмет не в белом списке
        ...
        if not a and b == 'инвентарь полон' then -- если контейнер заполнен
          while not robot.drop(3) do -- ждать, пока не освободится
			report{"coo6LLleHue_Ha_chatBox", b}
            sleep(30) -- подождать
          end
        end
      end
  ...
  if forcibly then
	report{"coo6LLleHue_Ha_chatBox", "поиск инструмента в контейнере"}
	...
	report{"coo6LLleHue_Ha_chatBox", "попытка ремонта инструмента"}
    if robot.durability() < 0.3 then -- если инструмент не заменился на лучший
            ...
            while true do
              ...
              if charge then
                if n_charge == max_charge then
				  ...
				  report{"coo6LLleHue_Ha_chatBox", "инструмент: ".. math.floor((n_charge+1)/max_charge*100).."% заряжен"}
                end
              else -- если инструмент не чинится
				report{"coo6LLleHue_Ha_chatBox", "инуструмент не заряжается"}
				stop_work()
              end
            end
          else
			report{"coo6LLleHue_Ha_chatBox", "инструмент не может быть отремонтирован"}
			stop_work()
          end
        else
          turn() -- повернуться
      ...
      while robot.durability() < 0.13 do
        report{"coo6LLleHue_Ha_chatBox", "нужен новый инструмент"}
        sleep(30)
      end
    end
  end
  if enderchest and not forcibly then
    ...
    while energy_level() < 0.98 do -- ждать полного заряда батареи
	  report{"coo6LLleHue_Ha_chatBox", "идет зарядка робота"}
      sleep(30)
    end
  end
  ignore_check = nil
  if not interrupt then
	report{"coo6LLleHue_Ha_chatBox", "возвращаюсь к работе"}
    go(0, -2, 0)
    go(x, y, z)
    smart_turn(d)
  end
...
for o = 1, 10 do -- цикл ограничения спирали
  for i = 1, 2 do -- цикл обновления координат
    for a = 1, o do -- цикл перехода по линии спирали
      ...
      --report() -- сообщить о завершении работы в чанке
	  report{"coo6LLleHue_Ha_chatBox", 'чанк #'..pos[3]+1 ..' завершен'}
      ...
      if pos[3] == chunks then -- если достигнут последний чанк
		...
		report{"coo6LLleHue_Ha_chatBox", computer.uptime()-Tau..' секунд\nходов: '..steps..'\nповоротов: '..turns}
      else -- иначе
        WORLD = {x = {}, y = {}, z = {}} 
