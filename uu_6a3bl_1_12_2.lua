local Ha3BaHue_o6HoBJIeHu9l = "создание текстбокса 6"
...
function creat_new_textBox(nick, x, y, w, h, label, name, visible, color_background, color_background2, color_background3, color_foreground, horizontal_Alignment)
	...
	caption.setVisible(visible)
	caption.setClickable(false)
	local background2 = MoHuTop_urpoka[nick].addBox(x, y + 1, w, h, color_background2)
	background2.setVisible(visible)
	background2.setClickable(false)
	local background3 = MoHuTop_urpoka[nick].addBox(x + 1, y + 1, w - 1, h - 1, color_background3)
...
function forms:creat_casino_form(nick)
	...
	--админские кнопки
	if nick == admin then
		table_form.zanucaTb_agpec_MM = creat_new_button(num_button(), nick, 5 + x_win, y_func(), 128, 15, "записать адрес ММ", "button", start_visible, gray, white, function() Ta6JIuca_admin_koMaHg[Ha3BaHue_6a3bl .. " казино записать адрес мм"]() end)
